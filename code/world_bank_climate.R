...
wb_adapt = subset(wb_adapt, year == 2022)
...
dat = merge(wb_adapt_agg, inform, by="iso3")
...
fit = lm(log(value)~inform, data=dat)
summary(fit)

p = ggplot(dat, aes(x=inform, y=value, color=inform, text=paste0(
