...
inform = read.xlsx("input/INFORM CC Brochure data.xlsx",
                   cols = c(1, 2),
                   startRow = 3)
names(inform) = c("recipient_name", "inform")
inform$iso3 = countrycode(
  inform$recipient_name,
  origin="country.name",
  destination="iso3c"
)
inform$iso3[which(inform$recipient_name=="Micronesia")] = "FSM"
inform$recipient_name = NULL
inform$year = 2022

# setdiff(unique(gain.m$iso3), unique(inform$iso3))
# setdiff(unique(inform$iso3), unique(gain.m$iso3))

gain.m$year = as.numeric(as.character(gain.m$year))
dat = merge(gain.m, inform, by=c("iso3", "year"))
...
fit = lm(inform~gain, data=dat)
summary(fit)

p = ggplot(dat, aes(x=gain, y=inform, color=gain, text=paste0(
  ...
  expand_limits(y=c(0, max(dat$inform*1.1))) +
  scale_color_gradient2(low=reds[2], high=reds[1]) +
  guides(color="none") +
  di_style + labs(
    x="Global Adaptation Initiative Country Index (ND-GAIN)",
    y="INFORM Climate Change Risk Index"
  )

ggsave(filename="output/climate_comparison.png", plot=p, width=10, height=5)
